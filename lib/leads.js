...
async function processContact(contact, engagements) {
  ...
  // The engagement must be timestamped on/after the contact's createdate —
  // returning customers often have prior meetings/emails/calls on file from
  // earlier interactions, and those must NOT count as a response to the
  // current lead. (Engagements are sorted ascending, so .find returns the
  ...
  // we allow engagements up to 30 minutes prior to createdate to count.
  // Real legacy engagements on returning contacts are always hours+ older
  // than the new lead, well outside this window.
