...
async function processContact(contact, engagements) {
  ...
  // Anchor the "lead start" at the latest of createdate and the most recent
  // form-fill conversion. Returning contacts that re-engage via a new form
  // keep their original 2025 createdate but get a fresh 2026
  // recent_conversion_date — for response classification and speed-to-lead
  // we treat the new conversion as the lead start, so engagements from the
  // prior inquiry don't get counted as a response to the new one.
  let leadStartMs = createDate.getTime();
  if (props.recent_conversion_date) {
    const rc = new Date(props.recent_conversion_date).getTime();
    if (!isNaN(rc) && rc > leadStartMs) leadStartMs = rc;
  }
  const leadStartDate = new Date(leadStartMs);

  // Find first meaningful response.
  // The engagement must be timestamped on/after the lead start — returning
  // customers often have prior meetings/emails/calls on file from earlier
  // interactions, and those must NOT count as a response to the current
  // lead. (Engagements are sorted ascending, so .find returns the earliest
  // qualifying one.)
  //
  // Grace window: for Call / Walk-in leads the rep often logs the call or
  // walk-in note seconds-to-minutes BEFORE creating the contact record, so
  // we allow engagements up to 30 minutes prior to lead start to count.
  const RESPONSE_GRACE_MS = 30 * 60 * 1000;
  const earliestAllowed = new Date(leadStartMs - RESPONSE_GRACE_MS);
  const firstResponse = engagements.find((e) => {
  ...
  let speedToLeadMinutes = firstResponseTime
    ? (firstResponseTime - leadStartDate) / (1000 * 60)
    : null;
  let speedToLeadBizMinutes = firstResponseTime
    ? calcBusinessMinutes(leadStartMs, firstResponseTime.getTime(), tz)
    : null;

  // Pending elapsed-clock fallback for non-responded, non-walk-in leads.
  // Counts business minutes from lead start to "now" (this cache snapshot),
  ...
  if (!firstResponseTime && !isWalkIn && leadStartMs) {
    ...
    const elapsedBiz = calcBusinessMinutes(leadStartMs, Date.now(), tz);
