...
const {
  ...
  upsertLeadRows, upsertEngagementRows, upsertEngagementsBulk, safeJsonStringify,
...
async function processContact(contact, engagements) {
  ...
  return {
    ...
    leadTemp: props.lead_temp || null,
    firstReferrer: props.hs_analytics_first_referrer || null,
    firstUrl: props.hs_analytics_first_url || null,
    firstTouchCampaign: props.hs_analytics_first_touch_converting_campaign || null,
        ...
        if (engs.length > 0) {
          ...
          try {
            ...
            for (let i = 0; i < engs.length; i += CHUNK) {
              ...
              batch.forEach((eng, idx) => {
                ...
                params.push(
                  ...
                  eng.direction || null, safeJsonStringify(eng),
            ...
            await client.query(
              ...
              [safeJsonStringify(engs), row.contact_id]
            );
...
async function manualRecheckLead(contactId) {
  ...
  const updResult = await pool.query(
    `UPDATE leads
      ...
      WHERE contact_id = $2
      RETURNING data`,
    [safeJsonStringify(engs), contactId]
  );
