...
function processContact(contact, engagements) {
  ...
  // For Walk-in leads: any NOTE on the contact counts as "responded"
  // (a rep adding a note proves the conversation was documented)
  const firstResponse = engagements.find(
    (e) => isMeaningfulResponse(e) || (isWalkIn && e.type === "NOTE" && (e.body || "").trim().length > 0)
  );
