...
async function _fetchFromHubSpot() {
  ...
  for (let start = 0; start < contacts.length; start += BATCH_SIZE) {
    ...
    // Release the contact objects we just processed — they're large
    // (HubSpot property bags) and no longer needed once leads[] has the
    // derived data.  This lets V8 GC them before the next batch.
    ...
    if (batchNum % 5 === 0) {
      logMem(`after batch ${batchNum}`);
      if (global.gc) {
        global.gc();
        logMem(`after GC batch ${batchNum}`);
      }
    }

    // Brief pause between batches to let Node's event loop tick, drain
    // native HTTP/TLS buffers, and keep /api/health responsive.
  ...
  // Run the same recompute as the DB-load path so both paths produce
  // identical responded / firstResponseTime / speedToLead* values. Today
  // processContact already applies the 24h grace window, so this is
  // mostly a no-op — but it keeps the two paths byte-identical and
  // future-proofs against drift if the recompute logic evolves
  // independently of processContact.
  const recomputed = _applyRecompute({ leads, byMarina });
  setCache("allLeads", recomputed);
  setDbCache("allLeads", recomputed).catch((err) =>
