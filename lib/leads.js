...
function processContact(contact, engagements) {
  ...
  // For Walk-in leads: any call engagement with non-empty notes counts as "responded"
  // (the notes prove the conversation was documented by a rep)
  const firstResponse = engagements.find(
    (e) => isMeaningfulResponse(e) || (isWalkIn && e.type === "CALL" && (e.body || "").trim().length > 0)
  );
