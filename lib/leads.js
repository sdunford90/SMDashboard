        ...
        filterGroups: [
          {
            filters: [
              ...
              { propertyName: "customer_type_2", operator: "NEQ", value: "Spam" },
...
async function _processContactBatch(contacts, { incremental = false } = {}) {
  ...
  for (let start = 0; start < contacts.length; start += BATCH_SIZE) {
    ...
    for (const r of processed) {
      if (r.status === "fulfilled" && r.value) batchLeads.push(r.value);
    }
  ...
  return { leads, newEngagementCount };
}
...
async function _fetchFromHubSpot({ forceFullRefresh = false } = {}) {
  ...
  const { leads: processedLeads, newEngagementCount } = await _processContactBatch(
    contacts,
  ...
  if (existingCache && processedLeads.length > 0) {
    const updatedMap = new Map(processedLeads.map((l) => [l.contactId, l]));
    const patchedLeads = existingCache.leads.map((l) => updatedMap.get(l.contactId) || l);
