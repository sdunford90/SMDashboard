...
async function processContact(contact, engagements) {
  ...
  // For Walk-in leads: a NOTE added on or after the contact's createdate counts as "responded"
  // — must be >= createdate so notes from previous visits / old interactions don't count
  const firstResponse = engagements.find(
    (e) =>
      isMeaningfulResponse(e) ||
      (isWalkIn &&
        e.type === "NOTE" &&
        (e.body || "").trim().length > 0 &&
        new Date(e.timestamp) >= createDate)
  );
  ...
  if (!firstResponseTime && !isWalkIn && createDate) {
    ...
    if (elapsedBiz > 0) {
      speedToLeadBizMinutes = Math.min(elapsedBiz, MAX_PENDING_BIZ_MINUTES);
      speedIsPending = true;
    }
  }
  ...
  if (lastInboundEngagement && lastInboundEngagement.type === "EMAIL") {
    try {
...
async function _fetchFromHubSpot() {
  ...
  // Process each contact with bounded parallelism. processContact may await an
  // AI classifier call (up to 5s), so we cap concurrency to keep refresh time
  // reasonable without hammering Anthropic.
  const processTasks = contacts.map((contact, i) => async () => {
    const engs =
  ...
  const processed = await withConcurrency(processTasks, 8);
