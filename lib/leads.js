...
async function _recheckUnrespondedEngagements() {
  try {
    ...
    console.log(`[eng-recheck] Re-fetching engagements for ${result.rows.length} unresponded leads with 0 engagements...`);
    let found = 0;
    ...
    if (found > 0) {
      ...
      if (cached && Array.isArray(cached)) {
        for (const cid of patchedContactIds) {
          const idx = cached.findIndex(l => l.contactId === cid);
          if (idx !== -1) {
