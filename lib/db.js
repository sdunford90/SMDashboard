...
async function setDbCache(key, data, ttlMs = DB_CACHE_TTL_MS) {
  try {
    await getPool().query(
      ...
      [key, safeJsonStringify(data), ttlMs]
    );
  } catch (err) {
...
async function ensureLeadsTables() {
  ...
  _leadsTablesReady = (async () => {
    ...
    const ddl = [
      ...
      // Engagement-watermark column added in Task #25 (engagement-watermark
      // incremental refresh). Tracks the latest engagement hs_lastmodifieddate
      // we have searched up to, independently from the contact-modified
      // watermark, so a partial failure on either side can be retried
      // without losing ground.
      `ALTER TABLE sync_state ADD COLUMN IF NOT EXISTS last_engagement_sync_at TIMESTAMPTZ`,
    ];
...
async function upsertLeadRows(leads) {
  ...
  for (let i = 0; i < leads.length; i += CHUNK) {
    ...
    batch.forEach((lead, idx) => {
      ...
      params.push(lead.contactId, safeJsonStringify(lead));
    });
    await p.query(
...
// Postgres' json/jsonb types reject literal NUL (\u0000) in strings even
// though they're valid JSON. HubSpot call notes and email bodies
// occasionally contain control chars (mostly NUL) that break the insert
// with "invalid input syntax for type json". Strip them defensively
...
// Bulk multi-contact engagement upsert. Used by the engagement-watermark
// incremental refresh (Task #25), which discovers engagements directly via
// hs_lastmodifieddate >= sinceDate searches and may have rows for many
// different contacts in a single batch.
//
// Each row: { engagementId, contactId, type, direction, data, timestamp }
async function upsertEngagementsBulk(rows) {
...
// Returns the subset of `contactIds` that already have a row in the leads
// table. Used by the engagement-watermark refresh path to decide which
// modified contacts are brand-new (need full per-contact engagement walk
// to backfill history) vs. already-synced (engagements covered by the
// bulk watermark search, no per-contact walk needed).
async function getExistingLeadIds(contactIds) {
  ...
  try {
    ...
    console.error("[leads-tables] getExistingLeadIds error:", err.message);
    return new Set();
  }
}

// Drift detection metric (Task #25). A non-zero count means leads are
// landing in the responded=false / 0-engagements state and not getting
// patched within 7 days — a regression in the sync pipeline that the
// dashboard surfaces on the diagnostics page before reps notice.
async function getDriftMetric() {
  try {
    await ensureLeadsTables();
    const result = await getPool().query(
      `SELECT COUNT(*)::int AS c
       ...
       WHERE (data->>'responded')::text = 'false'
         ...
         AND (data->>'createDate')::timestamptz >= NOW() - INTERVAL '7 days'`
    );
    return { count: result.rows[0].c };
  } catch (err) {
    console.error("[leads-tables] getDriftMetric error:", err.message);
    return null;
  }
}
...
async function getSyncState() {
  try {
    await ensureLeadsTables();
    const result = await getPool().query(
      `SELECT last_sync_at, contact_count, engagement_count, last_engagement_sync_at
         FROM sync_state WHERE key = 'hubspot'`
    );
    if (result.rows.length > 0) {
      return {
        ...
        // null on the very first run after this column was added — the
        // refresh path falls back to lastSyncAt in that case.
        lastEngagementSyncAt: result.rows[0].last_engagement_sync_at,
      };
    }
  } catch (err) {
...
// `engagementSyncTimestamp` is optional. When omitted the column is left
// unchanged so a refresh that didn't run the engagement-watermark search
// (e.g. full refresh) doesn't clobber the previous watermark.
async function setSyncState(contactCount, engagementCount, syncTimestamp, engagementSyncTimestamp) {
  try {
    await ensureLeadsTables();
    const ts = syncTimestamp ? new Date(syncTimestamp).toISOString() : new Date().toISOString();
    if (engagementSyncTimestamp !== undefined) {
      const ets = engagementSyncTimestamp
        ? new Date(engagementSyncTimestamp).toISOString()
        : null;
      await getPool().query(
        `INSERT INTO sync_state (key, last_sync_at, contact_count, engagement_count, last_engagement_sync_at)
         VALUES ('hubspot', $3::timestamptz, $1, $2, $4::timestamptz)
         ON CONFLICT (key) DO UPDATE
           SET last_sync_at = $3::timestamptz,
               contact_count = $1,
               engagement_count = $2,
               last_engagement_sync_at = $4::timestamptz`,
        [contactCount, engagementCount, ts, ets]
      );
    } else {
      await getPool().query(
