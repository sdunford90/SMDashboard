...
// Eligibility for the speed-to-lead signal. Mirrors isSpeedToLeadEligible in
// lib/leads.js — only responded Web Form / Digital leads with a real measured
// response gap count toward the speed average. Rep-initiated sources (Call /
// Walk-in / Referral) are trivially 0 and pending leads haven't responded yet,
// so both would distort the answered-speed signal.
function isSpeedSampleEligible(l) {
  if (!l) return false;
  if (l.leadSource !== "Web Form" && l.leadSource !== "Digital") return false;
  if (!l.responded) return false;
  if (l.speedIsPending) return false;
  return l.speedToLeadBizMinutes !== null && l.speedToLeadBizMinutes !== undefined;
}

// Compute raw sub-signals for one property given an array of lead objects
// (shape: { responded, leadSource, speedToLeadBizMinutes, speedIsPending, callsOutbound, callsLogged, callsLoggedWithNotes }).
