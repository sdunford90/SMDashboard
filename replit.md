...
- **Backfill recheck**: `POST /api/refresh?backfill=true` — runs `_recheckUnrespondedEngagements` with no time bound and no row cap (truly unbounded, every matching lead) to mop up everything that drifted into the responded=false / 0-engagements state. Used after upstream HubSpot association issues are resolved.
...
- **Connection pooling**: `instrumentation.js` (which runs first via Next.js's `instrumentationHook`) creates a single shared `https.Agent` (`keepAlive:true, maxSockets:6, maxFreeSockets:2`), monkey-patches `node-fetch` in `require.cache`, and assigns `https.globalAgent`. `lib/hubspot.js` reuses that same agent (no duplicate setup) and passes it to the SDK constructor. This prevents the SDK's internal codegen layer from opening a new TLS connection per API call.
- **Memory management** (Task #27): bodyPreview trimmed to 200 chars. Batch-level memory logging every 5 batches. Post-refresh `global.gc()` after each `forceRefresh()` to compact the JSON.stringify-heavy refresh path. Every 30 min an idle-socket sweep walks `agent.freeSockets` and destroys only the idle TLS sockets (active in-flight `agent.sockets` and the agent itself are left alone) so per-socket buffers (~tens of KB each) get freed back to the OS instead of accumulating across hours of uptime. Heartbeat (every 5 min) calls `global.gc()` if RSS crosses 1.2 GB.
- **Production start**: `MALLOC_ARENA_MAX=2 node --expose-gc --max-old-space-size=768 --max-semi-space-size=32 …`. `MALLOC_ARENA_MAX=2` is the single biggest non-V8 RSS lever — it caps glibc to one allocator pool instead of one per CPU × 8, which had been holding multi-GB of fragmented freed memory that never returned to the OS. Combined with the tighter V8 flags, total resident memory now stays under the 2 GB VM cap (target: ≤ 1.5 GB steady state) instead of the 2.5–4.7 GB spikes that were causing OOM kills.
